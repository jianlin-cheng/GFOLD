...
foreach $file (sort @files)
{
	...
	if(defined($native_image) and defined($native_info))
        {
        	`Rscript /data/jh7x3/GFOLD_v0.1/examples/scripts/add_info_in_image.R $native_dir/$filename.png $native_dir/$filename.info ${filename}_superimpose_rebuilt_scwrl.png ${filename}.eva ${filename}.png`;
