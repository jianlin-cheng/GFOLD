...
while(<IN>)
{
	...
	# assign (resid   2 and name ca) (resid  57 and name cb) 42.91 0.10 0.10
	$res1 = substr($line,14,3);
	$res1 =~ s/^\s+|\s+$//g;
	if(!exists($res_list{$res1}))
	{
		die "Wrong res id format $res1 1\n";
	}
	...
	$atom1 = uc(substr($line,27,2));
	$res2 = substr($line,38,3);
	$res2 =~ s/^\s+|\s+$//g;
	if(!exists($res_list{$res2}))
	{
		die "Wrong res id format $res2 2\n";
	}
	...
	print OUT "$res1-$AA1TO3{$res1_name}-$atom1\t$res2-$AA1TO3{$res2_name}-$atom2\tdistance\t$dist\n";
